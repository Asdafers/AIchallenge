...
BigQuery export runs live on Cloud Run (`BIGQUERY_DRY_RUN=false`) and has confirmed writes to `methodic_demo.win_loss_responses` (3 rows across multiple pipeline runs). Local development defaults to dry-run unless GCP credentials are configured.
